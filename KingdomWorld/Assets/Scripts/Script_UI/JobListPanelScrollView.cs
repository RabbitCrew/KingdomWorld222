...
public class JobListPanelScrollView : JobStringArr
{
    ...
    {
        if (GameManager.instance.ReturnTutorialPanel().StartTutorial && index != GameManager.instance.ReturnTutorialPanel().jobButtonIndex) { return; }
                ...
                if (jobListButnArr.Length > index && index > -1)
                {
                    ...
                    if (GameManager.instance.jobCountDic[(JobNum)(jobListButnArr[index].butnNum)] > 0)
                    {
                        ...
                        if (jobListButnArr[index].butnNum != 0)
                        {
                            ...
                            if (index2 != -1) { GameManager.instance.RestHuman.Remove(this.citizenInfoPanel.gameObject); }
                        }
